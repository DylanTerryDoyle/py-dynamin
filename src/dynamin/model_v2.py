...
class Modelv2:
    ...
    Market class
    ============
    Runs the DYNamic Agent-based MINskyan (DYNAMIN) model. 
    ...
    ----
        ...
        cfirm_defaults : NDArray
            total C-firm defaults per period
        
        kfirm_defaults : NDArray
            total K-firm defaults per period
        ...
        interest_rate : NDArray
            average bank loan interest rate
        
        bank_defaults : NDArray
            number of bank defaults per period
    ...
    -------
        ...
        update_database(self, t: int) -> None
        ...
        run(self, seed: int | None=None, data_collector: DataCollector | None=None) -> None
    """
    ...
    def __init__(self, params: dict) -> None:
        ...
        self.cfirm_productivity:    NDArray = np.zeros(shape=self.time)
        self.kfirm_productivity:    NDArray = np.zeros(shape=self.time)
        self.profits:               NDArray = np.zeros(shape=self.time)
        self.esl_gdp:               NDArray = np.zeros(shape=self.time)
        # firm data
        ...
        self.vacancy_ratio:         NDArray = np.zeros(shape=self.time)
        ...
        self.interest_rate:         NDArray = np.zeros(shape=self.time)
        ...
        self.cfirms:       list[ConsumptionFirm] = [ConsumptionFirm(x, self.dt, self.time, self.params, self.initial_output, initial_wage) for x in range(self.num_cfirms)]
        self.kfirms:       list[CapitalFirm] = [CapitalFirm(x, self.dt, self.time, self.params, self.initial_output, initial_wage) for x in range(self.num_kfirms)]
        self.banks:        list[Bank] = [Bank(x, self.dt, self.time, self.params) for x in range(self.num_banks)]
        self.households:   list[Household] = [Household(x, self.dt, self.time, self.params, initial_wage) for x in range(self.num_households)] 
        # total firms list
        ...
        self.deposits[0] = sum(household.deposits[0] for household in self.households)
    ...
    def defaults(self, t: int) -> None:
        ...
        # bank defaults
        for bank in self.banks:
            bank.determine_default(self.interest_rate[t-1], t)
            if bank.bankrupt:
    ...
    def compute_gini(self, t: int) -> float:
        ...
        """
        deposits = np.sort([household.deposits[t] for household in self.households])
        n = deposits.shape[0]
        index = np.arange(1, n+1)
        gini = ((np.sum((2 * index - n - 1) * deposits)) / (n * np.sum(deposits)))
        return gini
    ...
    def compute_esl(self, t: int) -> float:
        ...
        C_banks = C.sum(axis=1, keepdims=True)  # Bank total loans
        C_firms = C.sum(axis=0, keepdims=True).T  # Firm total loans
    ...
    def indicators(self, t: int) -> None:
        ...
        self.cfirm_productivity[t]    = sum(cfirm.productivity[t] for cfirm in self.cfirms) / num_cfirms if num_cfirms > 0 else 0.0
        self.kfirm_productivity[t]    = sum(kfirm.productivity[t] for kfirm in self.kfirms) / num_kfirms if num_kfirms > 0 else 0.0
        ...
        self.cfirm_nhhi[t]            = (sum((cfirm.market_share[t] ** 2) for cfirm in self.cfirms) - (1 / num_cfirms)) / (1 - (1 / num_cfirms)) if num_cfirms > 1 else 1.0
        self.kfirm_nhhi[t]            = (sum((kfirm.market_share[t] ** 2) for kfirm in self.kfirms) - (1 / num_kfirms)) / (1 - (1 / num_kfirms)) if num_kfirms > 1 else 1.0
        ...
        self.vacancy_ratio[t]         = sum(firm.vacancies[t] for firm in self.firms)/self.num_households
        self.gini[t]                  = self.compute_gini(t)
        self.deposits[t]              = sum(household.deposits[t] for household in self.households)
        # Bank Indicators
        self.bank_nhhi[t]             = (sum((bank.market_share[t] ** 2) for bank in self.banks) - (1 / self.num_banks)) / (1 - (1 / self.num_banks))
        self.interest_rate[t]         = sum(bank.loan_interest[t] for bank in self.banks)/self.num_banks
        # Lagged indicators
        if t < self.steps:
        ...
        # calculate ESL/GDP 
        self.esl_gdp[t]               = self.compute_esl(t) / self.nominal_gdp[t]
